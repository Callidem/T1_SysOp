...
// Tradução de endereço lógico para físico, usando GM se presente
static int translate_addr(CPU *cpu, int logical) {
    if (!cpu->gm) {
        return legal(cpu, logical) ? logical : -1;
    }
    int phys = end_fisico(cpu->gm, cpu->pageTable, cpu->pageTableSize, logical);
    if (phys < 0) {
        cpu->irpt = intEnderecoInvalido;
        return -1;
    }
...
void cpu_set_page_table(CPU *cpu, int *pageTable, int pageTableSize) {
    ...
    cpu->pageTableSize = pageTableSize;
}
...
int cpu_run_steps(CPU *cpu, int steps) {
    ...
    cpu->cpuStop = 0;
    int executed = 0;
    while (!cpu->cpuStop && executed < steps) {
        int phys_pc = translate_addr(cpu, cpu->pc);
        if (phys_pc < 0) break;
        cpu->ir = cpu->mem->pos[phys_pc];
        ...
        switch (cpu->ir.opc) {
            case LDI:
                ...
                break;
            case LDD: {
                int phys = translate_addr(cpu, cpu->ir.p);
                if (phys < 0) break;
                cpu->reg[cpu->ir.ra] = cpu->mem->pos[phys].p;
                cpu->pc++;
                break;
            }
            case LDX: {
                int addr = cpu->reg[cpu->ir.rb];
                int phys = translate_addr(cpu, addr);
                if (phys < 0) break;
                cpu->reg[cpu->ir.ra] = cpu->mem->pos[phys].p;
                cpu->pc++;
                break;
            ...
            case STD: {
                int phys = translate_addr(cpu, cpu->ir.p);
                if (phys < 0) break;
                cpu->mem->pos[phys].opc = DATA;
                cpu->mem->pos[phys].p = cpu->reg[cpu->ir.ra];
                cpu->pc++;
                break;
            }
            case STX: {
                int addr = cpu->reg[cpu->ir.ra];
                int phys = translate_addr(cpu, addr);
                if (phys < 0) break;
                cpu->mem->pos[phys].opc = DATA;
                cpu->mem->pos[phys].p = cpu->reg[cpu->ir.rb];
                cpu->pc++;
                break;
            }
            case MOVE:
            ...
            case JMPIM: {
                int phys = translate_addr(cpu, cpu->ir.p);
                if (phys < 0) break;
                cpu->pc = cpu->mem->pos[phys].p;
                break;
            }
            case JMPIG:
                if (cpu->reg[cpu->ir.rb] > 0) cpu->pc = cpu->reg[cpu->ir.ra]; else cpu->pc++;
                break;
            case JMPIGK:
                if (cpu->reg[cpu->ir.rb] > 0) cpu->pc = cpu->ir.p; else cpu->pc++;
                break;
            case JMPILK:
                if (cpu->reg[cpu->ir.rb] < 0) cpu->pc = cpu->ir.p; else cpu->pc++;
                break;
            ...
            case JMPIGM: {
                int phys = translate_addr(cpu, cpu->ir.p);
                if (phys < 0) break;
                if (cpu->reg[cpu->ir.rb] > 0) cpu->pc = cpu->mem->pos[phys].p; else cpu->pc++;
                break;
            }
            case JMPILM: {
                int phys = translate_addr(cpu, cpu->ir.p);
            ...
            case JMPIGT:
                if (cpu->reg[cpu->ir.ra] > cpu->reg[cpu->ir.rb]) cpu->pc = cpu->ir.p; else cpu->pc++;
                break;
            case DATA:
                cpu->irpt = intInstrucaoInvalida;
                break;
            case SYSCALL:
                if (cpu->sysCallIO) cpu->sysCallIO(cpu);
                cpu->pc++;
                break;
            case STOP:
                if (cpu->sysCallStop) cpu->sysCallStop(cpu);
                cpu->cpuStop = 1;
                break;
            case ___:
            default:
