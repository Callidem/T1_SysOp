...
void utils_load_program(Memory *mem, GM *gm, int progSize, const Word *image) {
    for (int i = 0; i < progSize; ++i) {
        // percorre uma palavra de cada vez, traduzindo o endereço lógico para físico
        int fisico = end_fisico(gm, i);
        if (fisico < 0) {
...
void utils_load_and_exec(CPU *cpu, Memory *mem, GM *gm, int progSize, const Word *prog) {
    utils_load_program(mem, gm, progSize, prog);
