...
int main(void) {
    ...
    printf("Vou alocar memoria para o programa %s (tamanho %d palavras)\n", progName, p->size);
    if (!aloca(p->size, &gm)) {
    ...
    if(gm.tabelaPaginas == NULL) {
        printf("Erro: tabela de páginas não alocada\n");
        memory_free(&mem);
        return 1;
    }

    for(int i = 0; i < gm.tamTabela; i++) {
    ...
    utils_load_and_exec(&cpu, &mem, &gm, p->size, p->image);  // Carrega programa e executa
