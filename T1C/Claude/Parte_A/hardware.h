...
typedef struct {
    Word *pos; ///cada palavra é uma posição de memória
} Page; 

typedef struct {
    int size; // Número total de palavras na memória
    Page *frames; // Array de frames da memória
...
struct CPU {
    ...
    // -----  P A G I N A Ç Ã O  -----
    // Tabela de páginas do processo - mapeia endereços lógicos para frames físicos
    int *tabela_paginas;  // Array com índices dos frames alocados
    int nro_paginas;      // Número de páginas (e de frames) alocadas para este processo
    int processo_id;      // ID do processo que está em execução
};
...
void memory_init(Memory *mem, int tamMem, int tamPg);  // Inicializa memória com tamanho
